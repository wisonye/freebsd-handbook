...
Add the following IP address to =/etc/mygate.conf= for the defualt gateway:
...
*** Configure PKG mirror and update
...
#+BEGIN_SRC bash
  ...
  #
  export PKG_PATH=https://mirror.fsmg.org.nz/pub/OpenBSD/7.4/packages/amd64/
#+END_SRC


After that, run the following command to update all installed packages:
...
#+BEGIN_SRC bash
  doas pkg_add git delta neovim py3-neovim bat ripgrep lf fish neofetch emacs-29.1p0-no_x11 cmake
#+END_SRC
