...
#+BEGIN_SRC bash
  doas pkg install tree git git-delta bat ripgrep procs htop btop fastfetch fzf fish neovim py39-pynvim unzip icu npm cmake tmux emacs-devel aspell aspell-ispell en-aspell lf
#+END_SRC
...
Then put =lua-language-server/bin= to your =$PATH=
...
First you need to download and install the lastest zig release from [[here][https://ziglang.org/download/]]

#+BEGIN_SRC bash
  #
  # Clone `zls` repo with the SAME zig release TAG you downloaded!!!
  # For example, `0.13.0`
  #
