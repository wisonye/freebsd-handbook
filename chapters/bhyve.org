...
For example, your local LAN network is =192.168.1.0/24=, the network topology would look like this:
...
If you want all VMs in an isolated LAN under your current (FreeBSD Bhyve Host) network, similar to a VPC, then you should create a VM switch in this way.
...
#+BEGIN_SRC conf
  192.168.2.10    my-alpine
  192.168.2.20    my-openbsd
#+END_SRC

Before ~scp~, you have to login to the vm and create ~~/.ssh~ folder first!!!
Before ~scp~, you have to login to the vm and create ~~/.ssh~ folder first!!!
Before ~scp~, you have to login to the vm and create ~~/.ssh~ folder first!!!
...
#+BEGIN_SRC bash
  scp -rv ~/.ssh/id_rsa.pub wison@my-alpine:/home/YOUR_USER_NAME_HERE/.ssh/authorized_keys
