...
After opeining =qutebrowser=, then run the following command to customize the key bindings:
...
When dealing with ~:set~, you got auto complection when pressing =tab= key, but you're able see the detailed help by running: ~:help XXXX~.
